...
void main(void){
    ...
    SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOB);    // Habilito reloj en el puerto B
    GPIOPinTypeADC(GPIO_PORTB_BASE, GPIO_PIN_5);    // Configuro PB.5 como entrada analógica
    GPIOPadConfigSet(GPIO_PORTB_BASE, GPIO_PIN_5, GPIO_STRENGTH_2MA, GPIO_PIN_TYPE_STD_WPD);    // Pull-Down en PB.5
    ADCSequenceConfigure(ADC0_BASE, 3, ADC_TRIGGER_PROCESSOR, 0); // Configura la fuente asociada a la CPU
    ADCSequenceStepConfigure(ADC0_BASE, 3, 0, ADC_CTL_IE | ADC_CTL_CH11 | ADC_CTL_END); // Habilitamos la forma del secuenciador (muestrea por secuencia, hab, int, del canal 11, la primera muestra es la última muestra)
